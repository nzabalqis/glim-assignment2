...
func main() {
	...
	http.HandleFunc("/signup", routeSignUp)
	http.HandleFunc("/logout", routeLogout)

	fmt.Println("server started at localhost:8050")
...
func routeIndex(w http.ResponseWriter, r *http.Request) {
	if r.Method == "GET" {
		...
		if !ok {
			var tmpl = template.Must(template.New("signin").ParseFiles("pages/signin.html"))
		...
		var tmpl = template.Must(template.New("home").ParseFiles("pages/home.html"))
...
func routeSignUp(w http.ResponseWriter, r *http.Request) {
	if r.Method == "GET" {
		var tmpl = template.Must(template.New("signup").ParseFiles("pages/signup.html"))
		var err = tmpl.Execute(w, nil)
